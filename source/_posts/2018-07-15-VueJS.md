...
不過唯一一點要注意的事情，就是綁定的是什麼元素，如：`input`、`Checkbox`、`radio`、`select`..等，
...
在呈現多筆資料時，可使用 `v-for` 功能，來實現重覆宣染畫面，

一般使用時，可使用 `v-for="item in list"`，

但若有需要資料的**索引值**的話，可改寫為 `v-for="(item, index) in list"`，

這樣就可以利用 `index` 取得目前的**索引值**。
